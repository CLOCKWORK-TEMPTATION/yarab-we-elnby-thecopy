...
OPTIONS:
  -AgentType <type>  Specific agent to update (claude|gemini|copilot|...)
...
$CURSOR_FILE   = Join-Path $REPO_ROOT '.cursor/rules/syskit-rules.mdc'
$QWEN_FILE     = Join-Path $REPO_ROOT 'QWEN.md'
$AGENTS_FILE   = Join-Path $REPO_ROOT 'AGENTS.md'
$WINDSURF_FILE = Join-Path $REPO_ROOT '.windsurf/rules/syskit-rules.md'
$KILOCODE_FILE = Join-Path $REPO_ROOT '.kilocode/rules/syskit-rules.md'
$AUGGIE_FILE   = Join-Path $REPO_ROOT '.augment/rules/syskit-rules.md'
$ROO_FILE      = Join-Path $REPO_ROOT '.roo/rules/syskit-rules.md'
...
$AGY_FILE      = Join-Path $REPO_ROOT '.agent/rules/syskit-rules.md'
$BOB_FILE      = Join-Path $REPO_ROOT 'AGENTS.md'
$VIBE_FILE     = Join-Path $REPO_ROOT '.vibe/agents/syskit-agents.md'
$KIMI_FILE     = Join-Path $REPO_ROOT 'KIMI.md'

$TEMPLATE_FILE = Join-Path $REPO_ROOT '.Systematize/templates/agent-file-template.md'
...
function Test-Environment {
    if (-not $CURRENT_BRANCH) {
        ...
        if ($HAS_GIT) { Write-Info "Make sure you're on a feature branch" } else { Write-Info 'Set SYSTEMATIZE_FEATURE environment variable or create a feature first' }
        exit 1
    }
    if (-not (Test-Path $NEW_PLAN)) {
        Write-Err "No plan.md found at $NEW_PLAN"
        Write-Info 'Ensure you are working on a feature with a corresponding sys directory'
        if (-not $HAS_GIT) { Write-Info 'Use: $env:SYSTEMATIZE_FEATURE=your-feature-name or create a new feature first' }
        exit 1
    }
    if (-not (Test-Path $TEMPLATE_FILE)) {
        Write-Err "Template file not found at $TEMPLATE_FILE"
        Write-Info 'Run node .Systematize/scripts/node/cli.mjs init or pwsh -File .Systematize/scripts/powershell/init-syskit.ps1 to scaffold .Systematize/templates, or add agent-file-template.md there.'
        exit 1
    }
}
