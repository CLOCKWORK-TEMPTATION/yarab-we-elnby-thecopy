...
function Write-PlatformSelectionScreen {
    ...
    for ($index = 0; $index -lt $PlatformStatuses.Count; $index++) {
        ...
        Write-Host "$pointer $marker $($platform.key) - $($platform.display_name) - status: $status"
            ...
            switch ($keyInfo.Key) {
                ([ConsoleKey]::UpArrow) {
                    if ($cursorIndex -gt 0) {
                        ...
                        $cursorIndex = $PlatformStatuses.Count - 1
                    }
                }
                ([ConsoleKey]::DownArrow) {
                    if ($cursorIndex -lt ($PlatformStatuses.Count - 1)) {
                        $cursorIndex++
                    } else {
                        $cursorIndex = 0
                    }
                ...
                ([ConsoleKey]::Spacebar) {
                    $platformKey = $PlatformStatuses[$cursorIndex].key
