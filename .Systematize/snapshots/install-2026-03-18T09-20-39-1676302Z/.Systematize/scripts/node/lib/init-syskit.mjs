...
function applyPlatformSelectionKey(state, key, platformStatuses) {
  ...
  if (key?.name === 'up') {
    nextState.cursorIndex = nextState.cursorIndex > 0 ? nextState.cursorIndex - 1 : platformStatuses.length - 1;
    return { done: false, state: nextState };
  }

  if (key?.name === 'down') {
    nextState.cursorIndex = nextState.cursorIndex < platformStatuses.length - 1 ? nextState.cursorIndex + 1 : 0;
    return { done: false, state: nextState };
  }

  if (key?.name === 'space') {
    const platformKey = platformStatuses[nextState.cursorIndex]?.key;
    if (platformKey) {
      if (nextState.selectedKeys.has(platformKey)) nextState.selectedKeys.delete(platformKey);
      else nextState.selectedKeys.add(platformKey);
    }
    return { done: false, state: nextState };
  }
...
function renderPlatformSelectionScreen(platformStatuses, state) {
  ...
  for (const [index, platform] of platformStatuses.entries()) {
    ...
    const isSelected = state.selectedKeys.has(platform.key);
    console.log(
      `${pointer} ${isSelected ? '[x]' : '[ ]'} ${platform.key} - ${platform.display_name} - status: ${translatePlatformStatusForPrompt(platform.status)}`
    );
