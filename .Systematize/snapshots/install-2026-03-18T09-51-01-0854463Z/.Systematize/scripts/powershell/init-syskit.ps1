...
function Get-PlatformSelectionEntries {
    param([Parameter(Mandatory = $true)][object[]]$PlatformStatuses)

    return @(
        [pscustomobject]@{ type = 'action'; action = 'select_all'; label = 'Select all' },
        [pscustomobject]@{ type = 'action'; action = 'unselect_all'; label = 'Unselect all' }
    ) + @($PlatformStatuses | ForEach-Object { [pscustomobject]@{
        type = 'platform'
        key = $_.key
        display_name = $_.display_name
        managed_outputs = $_.managed_outputs
        status = $_.status
    }})
}
...
function Write-PlatformSelectionScreen {
    param(
        [Parameter(Mandatory = $true)][object[]]$PlatformStatuses,
        [Parameter(Mandatory = $true)][System.Collections.Generic.HashSet[string]]$SelectedKeys,
        [Parameter(Mandatory = $true)][int]$CursorIndex,
        [string]$Message
    )

    $entries = @(Get-PlatformSelectionEntries -PlatformStatuses $PlatformStatuses)

    Clear-Host
    Write-Host 'Select the platforms to reinstall'
    Write-Host 'Use Up/Down to move, Space to toggle, and Enter to confirm.'
    Write-Host ''
    ...
    for ($index = 0; $index -lt $entries.Count; $index++) {
        ...
        if ($entry.type -eq 'action') {
            Write-Host "$pointer $($entry.label)"
            continue
        }

        $marker = if ($SelectedKeys.Contains($entry.key)) { '[x]' } else { '[ ]' }
        $status = Get-PlatformStatusPromptText -Status $entry.status

        Write-Host "$pointer $marker $($entry.key) - $($entry.display_name) - status: $status"
        Write-Host "    outputs: $($entry.managed_outputs -join ', ')"
    }
...
function Invoke-PlatformSelectionMenu {
    ...
    foreach ($platform in $PlatformStatuses) {
    ...
    try {
        ...
        Write-PlatformSelectionScreen -PlatformStatuses $PlatformStatuses -SelectedKeys $selectedKeys -CursorIndex $cursorIndex -Message $message
        ...
        while ($true) {
            ...
            if (($keyInfo.Modifiers -band [ConsoleModifiers]::Control) -and $keyInfo.Key -eq [ConsoleKey]::C) {
                throw 'Interactive selection cancelled by user.'
            }
            ...
            switch ($keyInfo.Key) {
                ...
                ([ConsoleKey]::DownArrow) {
                    if ($cursorIndex -lt ($entries.Count - 1)) {
                        $cursorIndex++
                    } else {
                        $cursorIndex = 0
                    }
                ...
                ([ConsoleKey]::Spacebar) {
                    $entry = $entries[$cursorIndex]
                    if ($entry.type -eq 'action') {
                        if ($entry.action -eq 'select_all') {
                            $selectedKeys.Clear()
                            foreach ($platform in $PlatformStatuses) {
                                $selectedKeys.Add($platform.key) | Out-Null
                            }
                        ...
                        if ($entry.action -eq 'unselect_all') {
                            $selectedKeys.Clear()
                        }
                    } else {
                ...
                ([ConsoleKey]::Enter) {
                    if ($selectedKeys.Count -eq 0) {
                        ...
                        Clear-Host
                        return @($PlatformStatuses | Where-Object { $selectedKeys.Contains($_.key) } | ForEach-Object { $_.key })
                    }
                }
            }
...
function Invoke-PlatformSelectionFallback {
    ...
    for ($index = 0; $index -lt $PlatformStatuses.Count; $index++) {
        ...
        $status = Get-PlatformStatusPromptText -Status $platform.status
        Write-Host "[$($index + 1)] $($platform.key) - $($platform.display_name) - status: $status"
        Write-Host "    outputs: $($platform.managed_outputs -join ', ')"
    }
    Write-Host 'Press Enter to select all platforms, or type numbers or keys separated by commas.'
    ...
    while ($true) {
        ...
        Write-Host 'Invalid selection. Use the listed numbers or platform keys.'
    }
}
...
function Resolve-SelectedPlatforms {
    ...
    try {
        $selectedKeys = Invoke-PlatformSelectionMenu -PlatformStatuses $platformStatuses
    } catch {
        if ($_.Exception.Message -eq 'Interactive selection cancelled by user.') {
            throw
        }
        $selectedKeys = Invoke-PlatformSelectionFallback -PlatformStatuses $platformStatuses
    }
