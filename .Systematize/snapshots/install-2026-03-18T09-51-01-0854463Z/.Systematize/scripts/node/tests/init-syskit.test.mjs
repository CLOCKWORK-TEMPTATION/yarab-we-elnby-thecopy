...
test('applies arrow and space selection keys for multi-select prompt state', () => {
  ...
  assert.deepEqual([...state.selectedKeys], ['codex', 'copilot']);

  ({ state } = applyPlatformSelectionKey(state, { name: 'return' }, platforms));
  assert.equal(state.message, '');
});

test('supports select all and unselect all action rows in the prompt', () => {
  const platforms = [
    { key: 'claude' },
    { key: 'codex' }
  ];
  ...
  ({ state } = applyPlatformSelectionKey(state, { name: 'up' }, platforms));
  ({ state } = applyPlatformSelectionKey(state, { name: 'space' }, platforms));
