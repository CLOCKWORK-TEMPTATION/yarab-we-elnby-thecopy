...
function buildPlatformSelectionEntries(platformStatuses) {
  return [
    { type: 'action', action: 'select_all', label: 'Select all' },
    { type: 'action', action: 'unselect_all', label: 'Unselect all' },
    ...platformStatuses.map((item) => ({ type: 'platform', ...item }))
  ];
}
...
function createPlatformSelectionState(platformStatuses) {
  return {
    cursorIndex: 1,
    selectedKeys: new Set(platformStatuses.map((item) => item.key)),
    message: ''
  };
}

function applyPlatformSelectionKey(state, key, platformStatuses) {
  const entries = buildPlatformSelectionEntries(platformStatuses);
  const nextState = {
  ...
  if (key?.name === 'up') {
    nextState.cursorIndex = nextState.cursorIndex > 0 ? nextState.cursorIndex - 1 : entries.length - 1;
    return { done: false, state: nextState };
  }

  if (key?.name === 'down') {
    nextState.cursorIndex = nextState.cursorIndex < entries.length - 1 ? nextState.cursorIndex + 1 : 0;
    return { done: false, state: nextState };
  }

  if (key?.name === 'space') {
    const currentEntry = entries[nextState.cursorIndex];
    if (currentEntry?.type === 'action') {
      if (currentEntry.action === 'select_all') {
        nextState.selectedKeys = new Set(platformStatuses.map((item) => item.key));
      }
      if (currentEntry.action === 'unselect_all') {
        nextState.selectedKeys = new Set();
      }
    } else if (currentEntry?.key) {
      if (nextState.selectedKeys.has(currentEntry.key)) nextState.selectedKeys.delete(currentEntry.key);
      else nextState.selectedKeys.add(currentEntry.key);
    }
    return { done: false, state: nextState };
  }
...
function renderPlatformSelectionScreen(platformStatuses, state) {
  ...
  console.log('Use Up/Down to move, Space to toggle, and Enter to confirm.');
  console.log('');

  for (const [index, entry] of entries.entries()) {
    const pointer = index === state.cursorIndex ? '>' : ' ';

    if (entry.type === 'action') {
      console.log(`${pointer} ${entry.label}`);
      continue;
    }

    const isSelected = state.selectedKeys.has(entry.key);
    console.log(
      `${pointer} ${isSelected ? '[x]' : '[ ]'} ${entry.key} - ${entry.display_name} - status: ${translatePlatformStatusForPrompt(entry.status)}`
    );
    console.log(`    outputs: ${entry.managed_outputs.join(', ')}`);
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const onKeypress = (_input, key = {}) => {
      if (key.ctrl && key.name === 'c') {
        cleanup();
        reject(new Error('Interactive selection cancelled by user.'));
        return;
      }

      const result = applyPlatformSelectionKey(state, key, platformStatuses);
      state = result.state;

      if (result.done) {
        const selectedKeys = platformStatuses
...
async function promptForPlatformSelectionsFallback(platformStatuses) {
  ...
  for (const [index, platform] of platformStatuses.entries()) {
    console.log(`[${index + 1}] ${platform.key} - ${platform.display_name} - status: ${translatePlatformStatusForPrompt(platform.status)}`);
    console.log(`    outputs: ${platform.managed_outputs.join(', ')}`);
  }
  console.log('Press Enter to select all platforms, or type numbers or keys separated by commas.');
  ...
  try {
    while (true) {
      ...
      console.log('Invalid selection. Use the listed numbers or platform keys.');
    }
  } finally {
    prompt.close();
  }
