...
import { CastMember, ExtendedCastMember, CastAnalysisResult } from '../../domain/models';
import {
  UserCircle, Star, User, HeartHandshake, Baby,
...
function generateCastingCall(members: CastMember[] | ExtendedCastMember[]): string {
  let documentText = 'CASTING CALL DOCUMENT\n';
  documentText += `${'='.repeat(50)}\n\n`;

  const leads = members.filter(member =>
    (member as ExtendedCastMember).roleCategory === 'Lead' || member.role === 'Lead'
  );
  const supporting = members.filter(member =>
    (member as ExtendedCastMember).roleCategory === 'Supporting' || member.role === 'Supporting'
  );
